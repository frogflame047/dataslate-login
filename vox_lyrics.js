// ==========================================
// WARP BLEED-THROUGH LYRIC ENGINE (FIREBASE EDITION)
// Track: JANE - THE LONG FACES (Shortened Cut)
// Ported from the ROBCO build. Timings, Firebase node and tuning
// keys are IDENTICAL - anything you already tuned live still applies.
// ==========================================
...
lyricStyle.innerHTML = `
    .lyric-glitch {
        ...
        text-shadow: 2px 0px 0px #2a4a8f, -2px 0px 0px #d8a33c !important;
    ...
    @keyframes textJitter {
        ...
        25% { transform: translate(-2px, 1px) skewX(2deg); color: #f2e6cf; }
        50% { transform: translate(2px, -1px) skewX(-2deg); color: #d8a33c; }
        75% { transform: translate(-1px, 2px) skewX(1deg); color: #8c1c13; }
    }
    @media (prefers-reduced-motion: reduce) {
        .lyric-glitch { animation: none !important; }
    }
`;
...
function triggerRealityGlitch(lyricObj, index) {
    ...
    if (isMinigameActive) {
        ...
        // Safe targets in the new build: scoreboard cells, dossier prose,
        // dossier table cells, section titles, vox-log slugs and purity seals.
        const safeSelectors = '#scoreboard-body td, #document-container p, #document-container th, #document-container td, .doc-section-title, .vox-slug, .seal-tag';
